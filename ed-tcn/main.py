...
if model_ == "SVM":
    ...
    for i in range(len(data_feat)):
        final = np.zeros((data_feat[i].shape[0],))
        label = data_labels[i]
        loc = data_labels_loc[i]
        for j in range(len(label)):
            final[loc[j]:loc[j+1]] = data_labels[i][j]
    ...
    p_test_f = [item for sublist in p_test for item in sublist]
    y_test_f = [item for sublist in y_test for item in sublist]
    
    print(accuracy_score(y_test_f, p_test_f))
